...
```json
{
  ...
  "SSID_Prefix": "wp",
  "Unit_s": "101,103",
  "Mots_de_passes": "4430jw6355$@,103jw!4430$",
...
- `Unit_s`, `Mots_de_passes`, `ssid_list`, and similar fields can be arrays, CSV strings, semicolon-separated strings, or newline-separated strings
- if `ssid_list` is not sent, the server builds SSIDs from `SSID_Prefix + Unit_s`
- old prefix rules are preserved: empty or `null` becomes `app`, while `None` and `Nada` become an empty prefix
